...
function detectType(title) {
  ...
  if (t.includes('ширма')) return 'ширма';
  if (t.includes('дельта')) return 'дельта';
  if (t.includes('лагуна')) return 'лагуна';
...
function formatDims(dims, type) {
  const useCross = ['куб', 'колонна', 'трапеция', 'ширма', 'дельта', 'лагуна', 'сектор', 'восьмигранник', 'черепашник'];
  const sep = useCross.includes(type) ? '×' : '*';
...
function generateExtraText(extra) {
  ...
  const map = {
    ...
    'поддон': `Также доступен вариант с поддоном. Его стоимость — ${price} руб.`,
  };
...
function generateDescription({ title, dims, glassThickness, bottomThickness, lidType, extra }) {
  ...
  if (type === 'куб') {
    ...
    p1 =
      `Аквариум ширма ${volume} л — стеклянная модель вытянутой горизонтальной формы, ` +
      `подходящая для содержания рыб, растений и нано-экосистем. Широкие пропорции создают ` +
      `эффект живой панорамы и делают аквариум выразительным элементом интерьера.`;
    p2 =
      `Габариты составляют ${dimsStr} мм, толщина стекла — ${glassThickness} мм, ` +
      `что обеспечивает надёжность конструкции. Высота указана без учёта крышки.`;
  } else if (type === 'дельта') {
    p1 =
      `Аквариум дельта угловой ${volume} л — стеклянная угловая модель со срезанными гранями, ` +
      `подходящая для содержания различных видов рыб и растений. Пятиугольная форма позволяет ` +
      `органично вписать аквариум в угол помещения и рационально использовать пространство.`;
    p2 =
      `Габариты составляют ${dimsStr} мм, толщина стекла — ${glassThickness} мм, ` +
      `что обеспечивает надёжность конструкции. Высота указана без учёта крышки.`;
  } else if (type === 'лагуна') {
    p1 =
      `Аквариум лагуна угловая ${volume} л — стеклянная угловая модель с изогнутой передней стенкой, ` +
      `подходящая для содержания различных видов рыб и растений. Плавные линии фронтального стекла ` +
      `обеспечивают панорамный обзор и придают аквариуму элегантный вид.`;
    p2 =
      `Габариты составляют ${dimsStr} мм, толщина стекла — ${glassThickness} мм, ` +
      `что обеспечивает надёжность конструкции. Высота указана без учёта крышки.`;
  } else if (type === 'сектор') {
    p1 =
      `Аквариум угловой сектор ${volume} л — стеклянная модель в форме четверти круга, ` +
      `подходящая для содержания различных видов рыб и растений. Округлая передняя стенка ` +
      `обеспечивает широкий панорамный обзор и позволяет органично вписать аквариум в угол помещения.`;
    p2 =
      `Габариты составляют ${dimsStr} мм, толщина стекла — ${glassThickness} мм, ` +
      `что обеспечивает надёжность конструкции. Высота указана без учёта крышки.`;
  } else if (type === 'восьмигранник') {
    p1 =
      `Аквариум восьмигранник ${volume} л — стеклянная модель восьмиугольной формы, ` +
      `подходящая для содержания рыб, растений и нано-экосистем. Многогранные стенки ` +
      `обеспечивают обзор с нескольких сторон и создают эффектный декоративный акцент в интерьере.`;
    p2 =
      `Габариты составляют ${dimsStr} мм, толщина стекла — ${glassThickness} мм, ` +
      `что обеспечивает надёжность конструкции. Высота указана без учёта крышки.`;
  } else if (type === 'черепашник') {
    p1 =
      `Аквариум черепашник ${volume} л — стеклянная модель, специально разработанная для содержания ` +
      `черепах и других полуводных животных. Прямоугольная форма обеспечивает комфортное размещение ` +
      `питомца и удобный уход.`;
    p2 =
      `Габариты составляют ${dimsStr} мм, толщина стекла — ${glassThickness} мм, ` +
      `что обеспечивает надёжность конструкции. Высота указана без учёта крышки.`;
  } else {
